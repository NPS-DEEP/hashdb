...
\textbf{New Database} \\
\begin{tabular}{p{3.6 in} p{3.0 in}}
\hangindent=2em \texttt{create [-b <block size>] [-s <sector size>] [<bloom settings>] <hashdb.hdb>} &
...
\textbf{Import} \\
\begin{tabular}{p{3.6 in} p{3.0 in}}
\hangindent=2em\texttt{import [-r <repository name>] [-w <whitelist.hdb>] [-s <t|f>] <hashdb.hdb> <import directory>} &
Import from path recursively into hash database, possibly suppressing low entropy hashes \\
...
\hangindent=2em\texttt{import\_dfxml [-r <repository name>] [-w <whitelist.hdb>] <hashdb.hdb> <dfxml.xml>} &
