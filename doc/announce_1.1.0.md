...
### Post-processing
New command `explain_identified_blocks` is added to support post-processing of the `identified_blocks.txt` file generated by _bulk\_extractor_.
The output consists of:

 * A table of hashes that are observed no more times than a threshold value, along with source information.
...
* Go through the `identifed_blocks.txt` file:
	- for every hash that matches no more than N files:
		- note the hash and all associated sources.
* For every hash identified:
	- display the hash and its associated source information.
* For every source identified:
	- display the source and its associated metadata.

Note that N is required.  This may be changed to permit a hardcoded value of 20.

Changes:

* New command `explain_identified_blocks <hashdb.hdb> <identified_blocks.txt> <number>` is added for post-processing analysis.
...
### Tuning
* New command `upgrade` is introduced to upgrade _hashdb_ v 1.0.0 databases to be compatible with 1.0.1.
