...
This command line example illustrates the process for looking up hash sources
...
Please see \hdb library file \texttt{hashdb.hpp}.
...
The command line example below demonstrates how to scan
image file \texttt{my\_imagefile}
for chunk hash values that match values present
in the \texttt{my\_hashdb} hash database and places
matches in the \texttt{outpath/identified\_blocks.txt} feature file:
...
The command line example below is similar, but uses Socket ...
