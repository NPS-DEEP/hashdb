...
using a \hmmi over database 1.
\item \texttt{rebuild\_bloom <bloom filter parameters> <hashdb>} \\
Rebuild the Bloom filters
using a \hmi over the given database.
\item \texttt{query\_hash <hashdb> <DFXML file>} \\
...
\begin{small}
\begin{verbatim}
pair<hash_map_manager_t::const_iterator, bool> insert(md5, uint64_t);
size_t erase(md5);
pair<hash_map_manager_t::const_iterator, bool> change(md5, uint64_t);
hash_map_manager_t::const_iterator find(md5);
bool has(md5);   // faster than find()
iterator begin();
iterator end();
size_t size();
\end{verbatim}
...
% hash multmap store
\subsection{Hash Multimap Manager}
...
\item A \texttt{const\_iterator} is available for walking the hash multimap.
\end{compactitem}

Here are the interfaces:
\begin{small}
\begin{verbatim}
pair<hash_multimap_manager_t::const_iterator, bool> insert(md5, uint64_t);
size_t erase(md5);
hash_multimap_manager_t::const_iterator find(md5, uint64_t);
bool has(md5, uint64_t);   // faster than find()
iterator begin();
iterator end();
size_t size();
\end{verbatim}
