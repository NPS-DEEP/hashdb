...
\textbf{md5deep} tool.
To download and install \textbf{md5deep} go to \url{http://github.com/simsong/hashdb/wiki/Installing-md5deep}.\\
...
\subsubsection{Importing to and Exporting from a Hash Database}

Now that the database has been created, the next step is to populate the database with hash values from a DFXML file. Commands to import and export hashes are shown in Table \ref{tab:importExport}.
...
Databases may need to be merged or common hash values may need to be subtracted out before they are used for scanning against.
These commands are outlined in Table \ref{tab:databaseManipulation}.
...
Statistics about database changes are reported on the console and to the log file and history file inside the hash database.  These statistics are shown in Table \ref{tab:changeStatistics}.
\begin{table}[!ht]
...
Various metadata statistics is available about a given hash database including the size of a database, where its hashes were sourced from, a histogram of its hashes, and more.
...
Analysis commands for analyzing \hash performance are available, see Table \ref{tab:analysis}.
...
\subsection{Using \hash with the \bulk Scanner}
\subsubsection{Importing Hashes and Scanning for Hashes}
The \bulk \hash scanner may be used to import hashes and to scan for hashes.
Basic \bulk scanner syntax is shown in Table \ref{tab:beScanner}.
