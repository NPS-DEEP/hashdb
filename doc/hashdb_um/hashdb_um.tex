...
\textbf{Command} & \textbf{Usage} & \textbf{Description} \\
\hline
\textbf{ingest} & \verb+ingest [-r <repository name>]+ \verb+[-w <whitelist.hdb>]+ \verb+[-s <step size>] [-e] <hashdb.hdb>+ \verb+<source directory>+& Computes and imports hashes from files under the source directory. Marks whitelist hashes. May process embedded data.\\
\hline
...
\hline
\textbf{scan\_image} & \verb+scan_image+ \verb+[-s <step size>]+ \verb+<hashdb> <media image>+ & Scans the hashdb for hashes that match hashes in the media image and prints out matches\\
\hline
\end{tabular}
\end{table}
...
\item Alternatively, use the ``copy\_range'' command to copy all hash values that have been imported some number of times, for example, exactly once.
\end{itemize}
...
\hline
\textbf{read\_bytes} & \verb+read_bytes <image file>+ \verb+<offset> <count>+ &  Prints raw bytes from a media image file\\
\hline
\end{tabular}
\end{table}
...
\verb+whitelist_dir, process_embedded_data, command_string)+\\
Recursively calculate and import hashes from path to \hdb.
\item \verb+error_message = scan_image(hashdb_dir, media_image_file, step_size,+\\
\verb+process_embedded_data)+\\
...
C++ syntax.  Read bytes from a media image file.
\item \verb+binary_string, bytes_read = read_bytes(media_image_file, offset, count)+\\
Python syntax. Read bytes from a media image file.
\end{itemize}
