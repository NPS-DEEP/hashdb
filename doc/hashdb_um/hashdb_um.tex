...
# hashdb version: 3.0.0-alpha-6
# username: bdallen
# start time 2016-07-20T17:02:12Z
{"name":"begin","delta":"0.000394","total":"0.000395"}
{"name":"end","delta":"0.000018","total":"0.000422"}
# command: "hashdb ingest KittyMaterial.hdb/ /home/bdallen/KittyMaterial"
# hashdb version: 3.0.0-alpha-6
# username: bdallen
# start time 2016-07-20T17:02:31Z
{"name":"begin","delta":"0.000353","total":"0.000353"}
# hashdb changes:
...
{"name":"end","delta":"8.475596","total":"8.475958"}
\end{lstlisting}
...
Although a \hdb hash database can become damaged, approaches may be used to avoid risk of loss. This section describes hazards, risks, safety provisions, and recommendations for preserving database integrity.\\

Use of database manipulation commands and import commands can easily result in incorrect source frequency count values for block hashes. The count value is correctly maintained when ingesting sources and when adding new block hashes to a database. \hdb is unable to correctly estimate count values when adding or ingesting hash values that are already present in the destination database. So if maintaining correct count values matters, do not import or use database manipulation commands when hash values overlap.\\

You can audit what has happened to a database by viewing the log file, see \textbf{\autoref{ContentsOfDB}}. If the audit log indicates that database manipulations have been applied and hash ranges overlap, then count values are likely to be larger than they should be.\\

If an operation is modifying a database, specifically, adding data to it, and the operation is aborted, transactions not flushed to disk will be lost, an end timestamp will not be present in the log file, and the database will be left functional but incomplete. Re-running the aborted operation will not cause the operation to fully complete.\\

To summarize:
\begin{itemize}
\item If accurate count values are required, please back up your database and be careful about what commands you run.  Inspect your database log to ensure that each change completed and that each change applied was acceptable. Recall that actions such as \verb+add_multiple+ can be valid if databases being added do not have overlapping block hash values. As a safeguard, \hdb will print the following warning message if it detects that a change could have compromised a count value:

\begingroup
\footnotesize
\begin{Verbatim}[fontfamily=courier]
Merge detected.  Count values are no longer accurate for this database.
\end{Verbatim}
\endgroup

\item If accurate count values are not required, use import and database manipulation commands as desired, but expect count values to be high and to see a warning message that count values will no longer be accurate.

\item
If your usage does not require completeness, feel free to abort commands that change databases, but expect that destination data will be incomplete even if the command is re-run.
...
\hline
\textbf{export\_range} & \verb+export_range <hashdb.hdb>+ \verb+<hashdb.json>+ \verb+<begin hex block hash>+ \verb+<end hex block hash>+& Exports block hashes in range to the JSON file. This command accepts a dash (\verb+-+) as a filename to allow terminal streaming to \verb+stdout+.\\
\hline
\end{tabular}
\end{table}
...
This command is likely to be discontinued in a future release of \hdb.
...
\item \textbf{export\_range}\\
Exports block hash values within the range and the source data associated with these block hashes from a \hdb block hash database. Data is in JSON format as described in \textbf{\autoref{ImportExportSyntax}}. The following example exports everything in \verb+demo.json+ to file \verb+example.json+.
\begin{Verbatim}[commandchars=\\\{\}]
\verbbf{hashdb export_range demo.hdb demo.json 00 ffffffffffffffffffffffffffffffff}
\end{Verbatim}
\end{itemize}
...
A block hash database may be sharded into multiple separate databases by using the ``export\_range'' command and then importing each range into individual shard databases.
...
By default, \hdb is compiled to calculate MD5 hashes. \hdb can be recompiled to use other encryption algorithms, please see source code file\\
