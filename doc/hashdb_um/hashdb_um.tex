...
The \hdb \verb+ingest+, \verb+scan_media+, and \verb+read_media+ commands support recursive extraction, meaning that they can recursively decompress compressed content. For \verb+ingest+, the result is that compressed data is uncompressed and submitted as a new file to be ingested. For \verb+scan_media+, the result is that compressed data is uncompressed and scanned. For \verb+read_media+, the forensic path is interpreted and the uncompressed content is returned. The recursive path to embedded data is called a forensic path. \hdb currently decompresses \textbf{zip} and \textbf{gzip} encodings.

\subsection{Forensic Path}
A forensic path is a byte offset into a media image. However, when data is uncompressed using recursive extraction, the forensic path includes information for pointing to uncompressed data and the compression algorithm used to reach that data. An example forensic path of \texttt{1000-zip-2100} at filename \texttt{myfile-100-zip} extracted from filename \texttt{myfile} is shown in Figure \ref{fig:forensicPath}.\\
