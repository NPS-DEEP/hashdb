...
\begin{figure}
	...
	\caption{Example screenshot of the \sscope main GUI}
...
The \textit{LMDB Hash Store} is a highly compressed optimized store of all the block hashes in the database.  When scanning for a hash, if it is not in this store, then it is not in the database.  Because of the degree of optimization, there can be false positives.  To compensate, \hdb reads the \textit{LMDB Hash Information Store} to be sure there is an actual match.
...
The \textit{LMDB Hash Store} has two tuning parameters: hash prefix bits and hash suffix bytes:

\begin{itemize}
\item \textit{hash prefix bits} is the number of bits of the hash prefix to use as the key in the store.  The idea is to select a value given the size of the database so that the average number of hashes with this prefix is slightly greater than 0, for example 20.  For example if you expect 5 billion hashes, you might select 28 because $5/2^{28}=18.6$, which is near 20.
\item \textit{hash suffix bytes} is the number of bytes of the hash suffix to store in the set of values of hash suffixes for this hash key.  The idea is to store as few suffix bytes as possible while minimizing false positives.  For example if you expect 5 billion hashes, you might select 3 bytes because $5/2^{28+8*3} = 5/2^{52} = 0.00011\%$ false positive rate, about 1 in 1 million.
\end{itemize}

Whenever a hash is found in the \textit{LMDB Hash Store}, \hdb reads the \textit{LMDB Hash Information Store} to make sure the hash really exists.  The \textit{LMDB Hash Store} should be tuned with an acceptably low false positive rate to minimize unnecessary accesses to the slower \textit{LMDB Hash Information Store}.
...
These tuning parameters help control the size of the \textit{LMDB Hash Information Store}:
\begin{itemize}
\item \verb+max_id_offset_pairs+ limits the number of source ID, file offset pairs that are stored for a hash, placing a cap on the maximum size of a `value`.
\item \verb+block_size+ is used to scale down the file offset value so that it fits in less bits of storage.  Additionally, all numbers in all LMDB stores are encoded using Protocol Buffers, ref. \url{https://developers.google.com/protocol-buffers/}.
\end{itemize}

\subsection{LMDB Source ID Store}
The \textit{LMDB Source ID Store} maps source file hash values to source IDs.  We use source IDs in the \textit{LMDB Hash Information Store} because they are significantly shorter than source file hashes.  We wouldn't need source IDs if we didn't make this optimization.
...
\begin{itemize}
\item The \verb+key+ is a \verb+source_id+.
