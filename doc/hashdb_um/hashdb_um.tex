...
\textbf{Command} & \textbf{Usage} & \textbf{Description} \\
\hline
\textbf{ingest} & \verb+ingest [-r <repository name>]+ \verb+[-w <whitelist.hdb>]+ \verb+[-s <step size>] [-x <rel>]+ \verb+ <hashdb.hdb> <source directory>+& Computes and imports hashes from files under the source directory. Marks whitelist hashes. Disable processing: \verb+r+ disable recursively processesing embedded data, \verb+e+ disable calculating block entropy, \verb+l+ disable calculating block labels.\\
\hline
