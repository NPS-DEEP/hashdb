...
Please note that \hash does not require DFXML files to import hashes.
The \bulk \hash scanner can import hashes directly into a new hash database,
see Section \ref{ImportingAndScanning} for importing using the \bulk \hash scanner.
...
For more examples of command usage, please see Section \ref{UseCases}.
For a \hash quick reference summary, please see Appendix \ref{QuickReference}
or \url{http://digitalcorpora.org/downloads/hashdb/hashdb_quick_reference.pdf}.

\subsection{General Usage}
Some commands display progress which can be suppressed using quiet mode,
...
\begin{table}[!ht]
\centering
\caption{Bloom Filter Settings for New Databases}
...
\hline
\textbf{\texttt{-B}} & \verb+--bloom_n <n>+ & Configures Bloom filter to work well for a database of $<$n$>$ hashes.\\
\hline
\textbf{\texttt{-C}} & \verb+--bloom_kM <k:M>+ & Available for advanced or experimental use.  Configures Bloom filter with k hash functions and M bits per hash.\\
\hline
\end{tabular}
\end{table}
...
\subsection{Importing and Exporting between a DFXML File and a Hash Database}
...
The \textbf{import} command in the above example imports the contents of \texttt{mock\_video.xml} into the database \textit{mock\_video.hdb}. \hash prints the following to the command line to indicate that the hashes have been inserted into the database successfully: 
...
\subsection{Manipulating Hash Databases}
...
Destination databases are created if they do not yet exist.
...
\hline
\textbf{add\_repository} &  add\_repository$<$source db1$>$ $<$destination db2$>$ $<$repository namedb$>$ & Adds \textit{source db1} to \textit{destination db2} but only when the repository name matches\\
\hline
...
\hline
\textbf{deduplicate} & deduplicate $<$source db$>$ $<$destination db$>$&   Copies all non-duplicate hash values from \textit{source db} into \textit{destination db}\\
\hline
\end{tabular}
\end{table}
...
\subsection{Scanning Media for Hash Values}
...
12464640 1d0abbddf1344ac751d17604bdd9ebe8 repository_name=mock_video_repos
...
\begin{itemize}
\item \texttt{hashdb\_mode} - the mode for the scanner. One of [none|import|scan] - ``none'' the scanner is active but performs no action, ``import'' - the scanner imports block hashes, ``scan'' - the scanner scans for matching block hashes
\item \texttt{hashdb\_block\_size} - Block size, in bytes, used to generate hashes. The default is 4096.
\item \texttt{hashdb\_ignore\_empty\_blocks} - Selects to ignore empty blocks.  One of [YES|NO].  The default is YES.
\item \texttt{hashdb\_scan\_path\_or\_socket} - The file path to a hash database or
...
\item \texttt{hashdb\_scan\_sector\_size} - Selects the scan sector size.  Scans along sector boundaries.
      Valid only in scan mode. Default value is 512.
\item \texttt{hashdb\_import\_sector\_size} - Selects the import sector size.  Imports along sector boundaries.
      Valid only in import mode. Default value is 4096.
\item \texttt{hashdb\_import\_repository\_name} - Selects the repository name to attribute the import to.
      Valid only in import mode. Default value is ``default\_repository''.
\item \texttt{hashdb\_import\_max\_duplicates} - The maximum number of duplicates to import
