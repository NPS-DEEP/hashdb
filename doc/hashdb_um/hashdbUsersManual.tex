...
\subsubsection{Installing Other Related Tools}
...
and \url{http://digitalcorpora.org/downloads/hashdb/hashdb_quick_reference.pdf}.
...
Table \ref{tab:hashDBSettings} shows the optional parameters that can be used to specify database settings.
% zzzz need table for Bloom settings
Bloom filter settings for performance tuning are not shown.\\
...
Except for the \textbf{deduplicate} command, the target database must already exist.
For the \textbf{deduplicate} command, if the target does not exist, one will be created with the same configuration settings as the source.
...
\includegraphics[scale=.8]{hashdb_quick_reference}
