...
In the above instructions, \texttt{\textbf{sample.hdb}} is thet empty database that will be created with default database settings.
...
\begin{table}[!ht]
\centering
\caption{Commands Available in \hash Command Line Tool to Create Hash Databases}
...
\centering
\caption{Database Changes Reported on the Console and Tracked in the \texttt{log.xml} File}
...
\begin{table}[!ht]
\centering
\caption{Commands Available in \hash Command Line Tool for Scanning Hash Databases}
...
\begin{table}[!ht]
\centering
\caption{Commands Available in \hash Command Line Tool to Obtain Statistics About Hash Databases}
...
\begin{table}[!ht]
\centering
\caption{Commands Available in \hash Command Line Tool for Tuning Hash Databases}
\label{tab:tuning}
\begin{tabular}{|p{3.5 cm}|p{6 cm}|p{4 cm}|}
\hline \hline
\textbf{Command} & \textbf{Usage} & \textbf{Description} \\
...
\item \texttt{hashdb\_scan\_max\_features} - The maximum number of feature lines to record or 0 for no limit.
      Valid only in scan mode. Default value is 0.
