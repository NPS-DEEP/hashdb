...
{
// this ifdef WIN32 may be factored out, see dfxml_writer.h
#ifdef WIN32
    fprintf(out,"hash_bytes: %I32u\n",b->hash_bytes);
    fprintf(out,"M: %I32u\n",b->M);
    fprintf(out,"k: %I32u32\n",b->k);
    fprintf(out,"vector_bytes: %I64u\n", (uint64_t)b->vector_bytes);
    fprintf(out,"vector_offset: %I32u\n",b->vector_offset);
    fprintf(out,"vector: %p\n",b->vector);
    fprintf(out,"# comment: %s\n",b->comment ? b->comment : "");
    fprintf(out,"added_items: %I64u\n",b->added_items);
    fprintf(out,"unique_added_items: %I64u\n",b->unique_added_items);
    fprintf(out,"aliased_adds: %I64u\n",b->aliased_adds);
...
int nsrl_bloom_create(nsrl_bloom *b,
		      ...
		      const char *comment)
{
    // Save copy of fname parameter
    b->fname = (char*)malloc(strlen(fname) + 1);
...
void nsrl_bloom_clear(nsrl_bloom *b)
{
    ...
    if(b->map_handle) {
        int status = unmap_file_region(b->map_handle);
        if (status != 0) {
            // program error
            assert(0);
        }
