...
#ifdef WIN32
// note that winsock2.h is not needed by bloom.hpp, but if winsock2.h is needed
// elsewhere, winsock2.h will complain if windows.h is already included.
#include <winsock2.h> // not needed by bloom.hpp but needed by windows.h
#include <windows.h>
#include <windowsx.h>
#include <wincrypt.h>
#endif
...
#else
int nsrl_bloom_open(nsrl_bloom *b,const char *fname,int mode) __attribute__ ((noreturn)); // abort
#endif
...
public:
    ...
    int create(const char *_fname,int _hash_size, int _M, int _k, const char *_comment){
	return nsrl_bloom_create(this,_fname,_hash_size,_M,_k,_comment);
    }
    int create(const char *_fname,int _hash_size, int _M, int _k, const std::string &_comment){
	return nsrl_bloom_create(this,_fname,_hash_size,_M,_k,_comment.c_str());
    }
#ifdef HAVE_PTHREAD
