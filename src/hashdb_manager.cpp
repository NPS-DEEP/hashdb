...
void usage() {
 ...
 << "\n"
  ...
  << "    Starts hashdb_manager as a lookup server service for supporting hashdb\n"
  << "    queries.\n"
  << "\n"
  << "    Options:\n"
  ...
  << "                    crash when performing concurrent lookups.\n"
  << "\n"
  << "    hash            Provides fastest lookup times and is very compact,\n"
  ...
  << "\n"
  << "Improving lookup speed by using sharding:\n"
  ...
  << "\n"
  << "Improving lookup speed by using Bloom filters:\n"
  << "Bloom filters can speed up performance during hash lookups by quickly\n"
  ...
  << "\n"
  << "Performing hash lookups using hashid scanner with bulk_extractor:\n"
  << "bulk_extractor may be used to scan for chunk hash values if the hashid\n"
  << "library is configured and enabled.  The hashid scanner runs either as\n"
  << "a client with hashdb_manager running as a server to perform hash lookups,\n"
  << "or loads the hash database directly and performs lookups directly.  The\n"
  ...
  << "\n"
  << "Performing hash lookups using the hashdb_checker tool:\n"
  ...
  << "running as a server hash database lookup service at a matching socket\n"
  ...
  << "persistent hash database open by starting a hash database lookup server\n"
  << "service and keeping it running.  Now this hash database will open quickly\n"
  << "for other lookup services because it will already be cached in memory.\n"
  ...
  << "\n"
  << "Known bugs:\n"
  << "Performing hash lookups in a threaded environment using the btree storage\n"
  ...
  << "recursive directories.  This bug is expected to go away in a future release\n"
  << "of boost btree.\n"
  << "\n"
  << "Examples:\n"
  ...
  << "\n"
  << "Once a hash database is created, it may be inspected using the \"--info\"\n"
  << "command.  Also the log file under the hash database directory may be\n"
  << "inspected.  This example displays information about hash database my_hashdb:\n"
  ...
  << "chunk hash values in a media file where the hash lookups are performed\n"
  << "locally from a hashdb database that is opened by the hashid scanner.\n"
  << "Parameters to bulk_extractor for this example follow:\n"
  << "\"-S lookup_type=use_path\" tells the scanner to perform hash lookups\n"
  << "using a hashdb at a local file path.\n"
  << "\"-S path=my_hashdb\" tells the scanner to perform hash lookups\n"
  ...
  << "sector boundary.\n"
  << "    bulk_extractor -S lookup_type=use_path\n"
  << "                   -S path=my_hashdb\n"
  << "                   -S chunk_size=4096\n"
  << "                   -S sector_size=512\n"
  ...
  << "for chunk hash values in a media file where the hash lookups are performed\n"
  << "remotely using a hash database lookup server service available at a socket\n"
  << "endpoint.  Parameters to bulk_extractor for this example follow:\n"
  << "\"-S lookup_type=use_socket\" tells the scanner to perform hash lookups\n"
  << "using a lookup server at a socket endpoint.\n"
  << "\"-S socket=tcp://localhost:14501\" sets the socket so that lookups use a\n"
  << "hashdb lookup server at socket endpoint \"tcp://localhost:14501\".\n"
  << "hashdb_manager must be running and available at\n"
  << "socket endpoint \"tcp://*:14501\" or else this example will hang waiting\n"
  << "for the server service to become available.  Please see the example for\n"
  << "starting hashdb_manager as a server lookup service.\n"
  << "\"-S chunk_size=4096\" tells the scanner to create hashes on 4096-byte\n"
  << "chunks of data.\n"
  << "\"-S sector_size=512\" tells the scanner to create hashes at every\n"
  << "512-byte sector boundary.\n"
  ...
  << "Specifically, the scanner will create hashes from chunk_size data at each\n"
  << "sector boundary.\n"
  << "    bulk_extractor -S path=my_hashdb\n"
  ...
  << "file my_dfxml match hash values in the hashdb used by the server service.\n"
  << "Parameters to the hashdb_checker tool follow:\n"
  << "\"--lookup\" tells hashdb_checker to perform a hash lookup.\n"
  << "\"-S tcp://localhost:14501\" tells the scanner to perform lookups using a\n"
  << "lookup server at socket endpoint \"tcp://localhost:14501\".  hashdb_manager\n"
  << "must be running and available at socket endpoint \"tcp://*:14501\" or else\n"
  << "this example will hang waiting for the server service to become available.\n"
  << "Please see the example for starting hashdb_manager as a server lookup service.\n"
  << "File \"my_dfxml\" is the name of the DFXML file containing hashes that will\n"
  << "be scanned for.\n"
  << "Output is directed to stdout.\n"
  << "    hashdb_checker --lookup -S tcp://localhost:14501 my_dfxml\n"
  << "\n"
  << "This example uses the hashdb_checker tool to display information about\n"
  << "the hashdb being used by a server lookup service.\n"
  << "Parameters to the hashdb_checker tool follow:\n"
  << "\"--info\" tells hashdb_checker to query the server service for information\n"
  << "about the hashdb that it is using.\n"
  << "\"-S tcp://localhost:14501\" tells the scanner to interact with the server\n"
  << "service using socket endpoint \"tcp://localhost:14501\".  hashdb_manager\n"
  << "must be running and available at socket endpoint \"tcp://*:14501\" or else\n"
  << "this example will hang waiting for the server service to become available.\n"
  << "Please see the example for starting hashdb_manager as a server lookup service.\n"
  << "Output is directed to stdout.\n"
  << "    hashdb_checker --info -S tcp://localhost:14501\n"
  << "\n"
  ;
}
