...
namespace hasher {
  ...
  static void* run(arg) {
    // get job_queue
    hasher::job_queue_t* job_queue = static_cast<hasher::job_queue_t*>(arg);

    while (job_queue->is_done == false) {
      hasher::job_t* job = job_queue->pop();
      if (job == NULL) {
        // queue is empty
        sched_yield();
      } else {
...
class threadpool_t {
  ...
  public:
  threadpool(const int num_threads, const job_queue_t* const p_job_queue) :
               threads(new ::pthread_t[num_threads], job_queue(p_job_queue) {

    // open one thread per CPU
    for (int i=0; i<num_threads; i++) {
      int rc = pthread_create(threads[i], NULL, hasher::run, (void*)job_queue);
    }
  }
